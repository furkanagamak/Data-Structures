...
/**
 * Date: 3/8/2022
 * This class describes a Maps which extends the Application class. It serves as the application for the Map menu
 * and includes the menu options for Maps.
...
public class Maps extends Application {
	...
	public void readCommand(Scanner keyboard) throws InvalidCommandException, EmptyStackException {
		...
		switch(input.toUpperCase()) {
			...
			case "P": // Plans a route.
				newCommand = new PlanRoute(keyboard);
				stk.pushCommand(newCommand);
				break;
				
			case "F": // Finds a place.
				newCommand = new FindPlace(keyboard);
				stk.pushCommand(newCommand);
				break;
			
			case "N": // Starts a navigation.
				newCommand = new StartNavigation(stk);
				stk.pushCommand(newCommand);
				break;
			
			case "S": // Switches to Safari.
				iCatchUp.option = "S";
				break;
